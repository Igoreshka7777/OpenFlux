    ...
    private var connectionControl: some View {
        VStack(spacing: 0) {
            ...
            Text(canStart ? "Нажмите на круг, чтобы \(vpn.active ? "отключить" : "подключить") VPN" :
                 "Добавьте ссылку VPN в настройках")
                .font(.system(size: 13))
                .foregroundColor(Theme.muted)
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 18) {
                        card {
                            ...
                            TextField("HTTPS-подписка или olcrtc://wbstream", text: $docURL)
                                .textInputAutocapitalization(.never)
                                .autocorrectionDisabled()
                                .keyboardType(.URL)
                                .textFieldStyle(.roundedBorder)
                                .disabled(vpn.active)
                            Text("Вставьте подписку из панели или прямую ссылку WB Stream. Прямая ссылка работает без загрузки HTTPS-подписки.")
                                .font(.caption)
                                .foregroundColor(Theme.muted)
                        }
                        card {
